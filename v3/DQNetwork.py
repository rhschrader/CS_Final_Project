...
class DQN:
    ...
    def train(self, Q_target, batch_size=32):
        ...
        with tf.GradientTape() as tape:
            ...
            # Calculate loss between new Q-value and old Q-value using huber loss
            loss = self.model.loss(updated_q_values, q_action)

        self.loss_history.append(loss.numpy()) # For plotting
        # Backpropagation
        grads = tape.gradient(loss, self.model.trainable_variables) # Calculate gradients
        self.model.optimizer.apply_gradients(zip(grads, self.model.trainable_variables)) # Apply gradients to model
